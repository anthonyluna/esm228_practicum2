...
**Target Population**: Products listed in the Seafood Watch "Best Choice" iist. We assume that farmed products are easier to meet compliance regarding traceability. This has an overall correlative impact such that $\rho = 0.8$. Products which are farmed and not farmed are based on the list provided by Sea Food Watch.https://www.seafoodwatch.org/-/m/sfw/pdf/guides/mba-seafoodwatch-west-coast-guide.pdf?la=en
...
population <- declare_population(
    ...
    product = add_level(N=rep(40,length(best_choice_categories)), product_farmed = category_farmed,
                        product_compliant = correlate(given=category_farmed, rho = 0.3,draw_binary,prob=.8))
)
...
```
**Challenge of drawing a representative sample**: Those with better track and trace standard operating procedures will be more open to reporting, whereas those with less effective standards may be less likely to agree to an audit for fear of discovery of inadequate documentation.
...
percent_strat <- reported_prop/sum(reported_prop)
percent_sampled <- .05

sampling <- declare_sampling(strata=category_farmed,
                 strata_n=pop.vector*percent_sampled*percent_strat)
